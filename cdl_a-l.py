...
def pure_membrane_q(t_hat):
    return np.sqrt(t_hat / 8.0)
...
def admissible_mask(t_hat, q, d_ratio):
    if np.isclose(d_ratio, 0.0):
        theta = theta_max_pure_membrane(t_hat)
    else:
        theta = np.full_like(t_hat, np.nan, dtype=float)
        internal = (t_hat > 0.0) & (q < 0.5 - 1e-6)
        theta[internal] = np.array(
            [
                theta_max_liquid(t_value, d_ratio, q_value)
                for t_value, q_value in zip(t_hat[internal], q[internal])
            ]
        )
        theta[q >= 0.5 - 1e-6] = H_OVER_L * np.pi**2 / 2.0
        if np.any(internal):
            theta[(t_hat <= 0.0) & (q < 0.5 - 1e-6)] = theta[internal][0]
...
def liquid_branch_curve(d_ratio):
    if np.isclose(d_ratio, 0.0):
        t_hat_plot = np.linspace(0.0, 2.0, 240)
        return t_hat_plot, pure_membrane_q(t_hat_plot)

    t_eq = t_hat_equipoise(d_ratio)
    t_hat_internal = np.linspace(1e-4, t_eq - 1e-3, 200)
    q_internal = np.array(
        [solve_q_liquid(t_hat, d_ratio) for t_hat in t_hat_internal]
    )
    q0 = pure_bending_q0(d_ratio)
    return np.r_[0.0, t_hat_internal, t_eq], np.r_[q0, q_internal, 0.5]
...
def make_plot():
    ...
    for d_ratio in D_RATIOS:
        t_hat_plot, q_plot = liquid_branch_curve(d_ratio)
        t_eq = t_hat_equipoise(d_ratio)
        label_x = 0.6 * t_eq if d_ratio < 1.0 else 0.55 * t_eq
        mask = admissible_mask(t_hat_plot, q_plot, d_ratio)
        ...
        equipoise_points.append((t_eq, 0.5))
        if d_ratio > 0.0:
            pure_bending_points.append((0.0, q_plot[0]))
    ...
    fig.savefig("fig_fixed_D_a_l.pdf", bbox_inches="tight")
    fig.savefig("fig_fixed_D_a_l.png", dpi=600, bbox_inches="tight")
    return fig, ax
